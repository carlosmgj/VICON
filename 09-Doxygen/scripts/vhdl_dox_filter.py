...
def main():
    ...
    # Buscar fichero de descripción asociado (.dox con mismo nombre)
    # Busca primero en DOX_EXTRA_CONTENT (recursivo), luego en la misma carpeta
    import os
    extra_content_dir = os.environ.get('DOX_EXTRA_CONTENT', '')
    desc_content = None
    
    if extra_content_dir:
        # Buscar recursivamente en 00-ExtraContent
        dox_name = path.stem + '.dox'
        for found in Path(extra_content_dir).rglob(dox_name):
            desc_content = found.read_text(encoding='utf-8', errors='replace')
            break  # usar el primero que encuentre
    
    if desc_content is None:
        # Fallback: misma carpeta que el .vhd
